...
# 各リゾートのモデルとCSVデータのパスを管理する辞書
# ★★★ ファイル名に合わせて、この辞書を設定します ★★★
RESORT_DATA = {
    "野沢": {
...
def create_comparison_bar_chart(forecast, historical_df):
    ...
    # スキーシーズンの定義（例: 11月から翌年4月）
    winter_months = [11, 12, 1, 2, 3, 4]
    df = df[df['ds'].dt.month.isin(winter_months)]

    # シーズンを定義する関数 (例: 2023年11月 -> 2023-24シーズン)
    def get_season(date):
        if date.month >= 11:
